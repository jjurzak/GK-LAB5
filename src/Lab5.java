...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    private final GLUT glut = new GLUT(); // An object for drawing GLUT shapes.
    ...
    private void spiral(GL2 gl2) {
        ...
        for (int i =0; i<340; i++) {
            ...
            float y = (float) (2 * Math.sin(angle));
            float z = 0.05f * i;
    ...
    private void triangle(float angle1, float angle2, int segments, GL2 gl2) {
        gl2.glBegin(GL_TRIANGLE_FAN);
        gl2.glVertex3d(Math.cos(angle1 * 2 * Math.PI / segments),
                Math.sin(angle1 * 2 * Math.PI / segments), 2);
        gl2.glVertex3d(Math.cos(angle2 * 2 * Math.PI / segments),
                Math.sin(angle2 * 2 * Math.PI / segments), 2);
    ...
    private void pyramid(GL2 gl2) {
        ...
        for (int i = 0; i < segments; i++) {
            float angle1 = i - 1;
            float angle2 = i;
            triangle(angle1, angle2, segments, gl2);
        }
    }
